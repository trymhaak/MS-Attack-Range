...
cat > attack-range.sh << 'EOF'
#!/bin/bash
# Wrapper script that activates virtual environment
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
source "$SCRIPT_DIR/.venv/bin/activate"
python "$SCRIPT_DIR/attack-range.py" "$@"
EOF

chmod +x attack-range.sh
...
echo "./attack-range.sh build"
echo ""
echo "For more information and commands, run:"
echo "./attack-range.sh --help"
